...
variaveis = {
    ...
    'jogo'     : f"insert into jogo(nome_jogo,desenvolvedora,classificacao_indicativa,categoria,sinopse,data_lancamento,IDadministrador) values ('%s','%s','%s','%s','%s','%s','%s')",
    'atualizajogo' : f"UPDATE %s SET  desenvolvedora = '%s',  classificacao_indicativa = '%s', categoria='%s', sinopse='%s',data_lancamento='%s', WHERE IDjogo = %s"
}
...
def seleciona_um(id, select ="*", tipo = 'administrador', atributo = 'IDadministrador'):
    try:
        ...
        print("seleciona_um=",variaveis["seleciona_um"]%(select,tipo, atributo, id))
...
def adiciona(tipo, data):
    val = valida(tipo, data) if (tipo == "usuario" or tipo == "administrador" or tipo == "jogo") else 200
